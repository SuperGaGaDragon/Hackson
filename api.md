...
## brief intro
- This document is the current verified API and port map for the Parallex public product on Hackson-named infrastructure.
- Public product traffic now uses `https://hackson.catachess.com/`.
- Public UI brand is `Parallex`; legacy Hackson names remain in service names, database names, token keys, and the confirmed public domain until a separate infrastructure migration.
...
| Public frontend assets after Work Command Rail and Progress Filters promotion | `/assets/index-C3qfrcZd.js`, `/assets/index-Cg6HDlAd.css` |
| Public frontend assets after Work Deliverable and Reliability risk-copy promotion | `/assets/index-B27cPK6C.js`, `/assets/index-DPQ_53Wg.css` |
| Public frontend assets after Work Quality side-rail promotion | `/assets/index-Dw8F-JPt.js`, `/assets/index-D-1voGoI.css` |
| Public frontend assets after TMLS Hackathon landing promotion | `/assets/index-BIYuo_LQ.js`, `/assets/index-BQKHVXzG.css` |
| Public frontend assets after Hackathon landing narrative refactor | `/assets/index-DolvLzft.js`, `/assets/index-BE51Xslx.css` |
| Public frontend assets after Parallex landing redesign | `/assets/index-DumYXDry.js`, `/assets/index-CTAU9a0y.css` |
| Public frontend assets after Quick Try intro-return and English persona defaults | `/assets/index-xZFoomKR.js`, `/assets/index-bb_rj1B3.css` |
| Public frontend assets after Parallex exhibition landing polish | `/assets/index-mGZNbVK8.js`, `/assets/index-B5mIAlaF.css` |
| Public frontend assets after Parallex capability hero polish | `/assets/index-C5UkgsN1.js`, `/assets/index-EDKtaC5D.css` |
| Public frontend assets after Parallex workflow-memory landing polish | `/assets/index-D3i-OEZL.js`, `/assets/index-n7oh3Jzg.css` |
| Public frontend assets after Work Mission Map and Quality modal promotion | `/assets/index-BSK8jMVG.js`, `/assets/index-Oy1RP9X9.css` |
| Public frontend assets after Parallex landing density polish | `/assets/index-DD16WV4J.js`, `/assets/index-Bh5JOroX.css` |
| Public frontend assets after Parallex minimal landing copy polish | `/assets/index-CPuWRddH.js`, `/assets/index-BFd_mbUZ.css` |
| Public frontend assets after Idle right rail polish | `/assets/index-B3JO7D8q.js`, `/assets/index-ZuQezEFt.css` |
...
- On 2026-05-28, public `8145` static frontend assets were updated with the Desktop Companion Mac alpha download page and route paths. The previous public `frontend/dist` was backed up to `~/hackson_domain_8145/frontend/dist.backup_desktop_companion_20260528142632`; only `frontend/dist` was replaced, and `hackson-domain-8145.service` was not restarted. Local verification passed with frontend build assets `/assets/index-BtKsAU0T.js` and `/assets/index-CFlhTWvI.css`, Desktop Pet `npm test` `10` tests, Tauri `.app` build, app zip packaging at `/assets/downloads/hackson-pet-mac-arm64.zip`, backend static frontend tests `4 passed`, local Playwright download/routes smoke, and public Playwright download/routes smoke. Public `https://hackson.catachess.com/download/companion`, `/idle`, `/companion`, `/work`, `/me`, `/work_project/not-real`, and `/assets/downloads/hackson-pet-mac-arm64.zip` returned usable pages or `200` download responses; public services `hackson-domain-8145.service` and `hackson-cloudflared.service` remained active. Screenshots: `scripts/artifacts/companion_download_public_desktop.png` and `scripts/artifacts/companion_download_public_mobile.png`.
- On 2026-05-28, public `8145` was updated with the Work Command Rail, running Mission instruction events, compact Mission rows, and Progress filters. The selected Project rail now separates Directory from Composer, Mission Header no longer owns text entry, running Missions can persist `USER_INSTRUCTION_ADDED`, and Progress supports `All`, `Thinking`, `Reliability`, `Products`, `Windows`, `Search`, `Inputs`, and `Issues` filters. Local verification passed with Work Mode `126` unittest tests, frontend build, and `git diff --check`. Public-directory Work Mode `126` unittest tests and frontend build passed before restart. After waiting for the active public Work Mission `6a1892c082ad5420d3164694` to finish safely as `blocked`, `hackson-domain-8145.service` was restarted. Local and public `/health` returned `{"status":"ok"}`, public root served `/assets/index-C3qfrcZd.js` and `/assets/index-Cg6HDlAd.css`, `/api/work/missions/{missionId}/instruction` was verified on a draft Mission with the expected `409 mission_not_running`, public Playwright UI smoke passed without starting a real model loop, screenshots were saved to `scripts/artifacts/work_command_rail_public.png` and `scripts/artifacts/work_command_rail_public_mobile.png`, production logs showed no traceback or 500, and Mongo checks showed `running_missions=0`, `running_runs=0`, and no orphan `codex exec` process.
- On 2026-05-28, public `8145` was updated with the Work authoritative Deliverable contract and Reliability risk-score copy. Product now distinguishes `latestArtifactId` for lineage recency from `deliverableArtifactId` for the user-facing answer, tracks `deliveryStatus`, keeps review/reliability/discussion Artifacts in Product History, and marks blocked best candidates as `blocked_candidate` instead of hiding them. Reliability reports now include `objective=false`, `scoreMeaning`, `confidence`, and `confidenceReason`; UI labels the score as a trace-backed risk score, not proof. Local verification passed with Work Mode `129` unittest tests, frontend build, Product reader smoke, and `git diff --check`. Public-directory verification passed with Work Mode `129` unittest tests and frontend build using Node `20.19.6`. After confirming `running_missions=0` and `running_runs=0`, `hackson-domain-8145.service` was restarted. Local and public `/health` returned `{"status":"ok"}`, public root served `/assets/index-B27cPK6C.js` and `/assets/index-DPQ_53Wg.css`, public Product reader and Command rail Playwright smokes passed with desktop/mobile screenshots in `scripts/artifacts/`, production logs showed no traceback, 500, `schema_invalid`, `discussion_result_invalid`, `turn_budget`, or errors, and Mongo checks showed `running_missions=0`, `running_runs=0`, `failed_recent=0`.
- On 2026-05-28, public `8145` was updated with Work Quality side-rail IA. Reliability no longer renders as an always-expanded main Mission card; the main order is now Activity, Product, Windows, Progress, and Diagnostics. The latest Reliability report appears as a compact right-rail `Quality` status with score, status, confidence, issue count, evidence count, and collapsed `Review details`. Progress still keeps the Reliability filter and evaluator events. Local verification passed with Work Mode `129` unittest tests, frontend build, Product reader smoke, and `git diff --check`. Public-directory verification passed with Work Mode `129` unittest tests and frontend build using Node `20.19.6`. After confirming `running_missions=0` and `running_runs=0`, `hackson-domain-8145.service` was restarted. Local and public `/health` returned `{"status":"ok"}`, public root served `/assets/index-Dw8F-JPt.js` and `/assets/index-D-1voGoI.css`, public Command rail and Product reader Playwright smokes passed with desktop/mobile screenshots in `scripts/artifacts/`, production logs showed no traceback, 500, `schema_invalid`, `discussion_result_invalid`, `turn_budget`, or errors, and Mongo checks showed `running_missions=0`, `running_runs=0`, `failed_recent=0`.
- On 2026-05-28, public `8145` was updated with the TMLS Agentic Hackathon root landing and Quick Try temporary-account path. Logged-out `GET /` now renders the QR-code landing with the Desktop Pet cat, `Quick Try`, `Create Account`, and concise product explanation; authenticated users keep the existing app shell. `POST /api/users/quick-try` creates a real temporary backend user, returns the normal JWT shape, and the frontend stores that token in session storage. Local verification passed with user service/route tests `11`, Work Mode regression `129`, frontend build, and `git diff --check`. Target public-directory verification passed with the same user and Work Mode tests plus frontend build using Node `20.19.6`. After confirming `running_missions=0` and `running_runs=0`, `hackson-domain-8145.service` was restarted. Local `/health` returned `{"status":"ok"}`, public root served `/assets/index-BIYuo_LQ.js` and `/assets/index-BQKHVXzG.css`, public Hackathon landing smoke passed on desktop/mobile with no horizontal overflow and Quick Try session storage verified, public Work command rail smoke still passed, production logs showed no traceback/500/errors, and Mongo checks showed `running_missions=0`, `running_runs=0`, `failed_recent=0`, with temporary Quick Try users created under `is_temporary=true`.
- On 2026-05-28, public `8145` was updated with the Hackathon landing narrative refactor. The logged-out root page now presents Hackson as an agent supervision runtime, separates core runtime value from entry surfaces, and uses the visible loop `Brainstorm -> Mission -> Product -> Quality -> Memory` without overclaiming unshipped Idle-to-Mission or Companion Work-context attachment features. Local frontend build and `git diff --check` passed. Target public-directory frontend build passed with Node `20.19.6`. After confirming `running_missions=0` and `running_runs=0`, `hackson-domain-8145.service` was restarted. Local `/health` returned `{"status":"ok"}`, public root served `/assets/index-DolvLzft.js` and `/assets/index-BE51Xslx.css`, public Hackathon landing smoke passed on desktop/mobile with no horizontal overflow and Quick Try session storage verified, public Work command rail smoke still passed, screenshots were saved to `scripts/artifacts/hackathon_landing_public*.png`, and production logs showed no traceback/500/errors.
- On 2026-05-28, public `8145` was updated with the Parallex landing redesign. Logged-out root now uses the user-facing Parallex brand, browser title `Parallex`, a typography-led hero, CSS-built Mission Runtime preview, and no Desktop Pet cat image on the landing page. Auth entry, app shell, and Desktop Companion download visible brand labels were aligned to Parallex while existing Hackson service/domain names stayed unchanged. Local frontend build, local desktop/mobile Playwright screenshots, and `git diff --check` passed. Target public-directory frontend build passed with Node `20.19.6`. After confirming `running_missions=0` and `running_runs=0`, `hackson-domain-8145.service` was restarted. Local `/health` returned `{"status":"ok"}`, public root served `/assets/index-DumYXDry.js` and `/assets/index-CTAU9a0y.css`, public Hackathon landing smoke passed on desktop/mobile with title validation, no horizontal overflow, no cat image, and Quick Try session storage verified. Screenshots were saved to `scripts/artifacts/hackathon_landing_public*.png`; production logs showed no traceback/500/errors and Mongo checks showed `running_missions=0`, `running_runs=0`, `failed_recent=0`.
- On 2026-05-28, public `8145` was updated with Quick Try intro-return and English default persona cleanup. Temporary Quick Try users now receive public `isTemporary=true`, see a top-bar `Back to intro` action, and that action clears the session-scoped quick token before returning to the public landing. New registered and Quick Try users now default to `languagePreference=en`; Quick Try profile text references Parallex; backend Agent catalog and frontend fallback Nora/Vale `personality`, `speaking_style`, and story brand text are English/Parallex while known old Chinese/Hackson defaults are normalized on read. Local verification passed with targeted backend unittest `16` tests, frontend build, and `git diff --check`. Target public-directory verification passed with targeted backend unittest `16` tests using `backend/.venv`, frontend build using Node `20.19.6`, and public assets `/assets/index-xZFoomKR.js` plus `/assets/index-bb_rj1B3.css`. After confirming `running_missions=0` and `running_runs=0`, `hackson-domain-8145.service` was restarted. Public Hackathon landing smoke passed with Quick Try `Back to intro` session-clearing verification, public API smoke confirmed `isTemporary=true`, `languagePreference=en`, and English Nora/Vale default personalities, logs showed no traceback/500/errors, and Mongo checks showed `running_missions=0`, `running_runs=0`, `failed_recent=0`.
- On 2026-05-28, public `8145` was updated with the Parallex exhibition landing polish. The logged-out root page now removes the repeated long-text card stack in favor of a sparse judge-facing brand moment, one-line positioning, a CSS-built product-state stage, the compressed `Intent -> Mission -> Product -> Review` loop, compact proof metrics, and four short signal rows. The page keeps Quick Try, Create Account, and temporary-user Back-to-intro behavior unchanged while improving desktop and mobile readability. Local frontend build, local desktop/mobile Playwright screenshots, and `git diff --check` passed. Target public-directory frontend build passed with Node `20.19.6`. After confirming `running_missions=0` and `running_runs=0`, `hackson-domain-8145.service` was restarted. Public root served `/assets/index-mGZNbVK8.js` and `/assets/index-B5mIAlaF.css`; public Hackathon landing smoke passed on desktop/mobile with title validation, no horizontal overflow, no cat image, and Quick Try Back-to-intro session clearing verified. Screenshots were saved to `scripts/artifacts/hackathon_landing_public*.png`; production logs showed no traceback/500/errors and Mongo checks showed `running_missions=0`, `running_runs=0`, `failed_recent=0`.
- On 2026-05-28, public `8145` static frontend assets were updated with the Parallex capability hero polish. The hero's right side no longer uses the abstract `Mission 042 / Trace / Risk` stage; it now shows a direct capability index for `Idle Mode`, `Work Mode`, `Evaluator`, and `Product History`, with one scan-friendly value line per capability. Local frontend build, local desktop/mobile Playwright screenshots, and `git diff --check` passed. Because public Mongo showed `running_missions=1` and `running_runs=1`, only `frontend/dist` and matching frontend source/docs/smoke files were synced; `hackson-domain-8145.service` was not restarted. Public root served `/assets/index-C5UkgsN1.js` and `/assets/index-EDKtaC5D.css`, public Hackathon landing smoke passed on desktop/mobile with Quick Try Back-to-intro session clearing verified, production logs showed no traceback/500/errors, and Mongo checks showed `failed_recent=0` while the pre-existing running Mission/Run remained active.
...
- On 2026-05-29, public `8145` static frontend assets were updated with Idle right rail product polish. Idle no longer shows raw `ID` or `Count` as default right-rail cards; Brainstorm now appears before compact Session status; conversation id, parent id, and turn count live under collapsed Debug; normal Loading/Working state is not duplicated below Debug. The previous public `frontend/dist` was backed up to `~/hackson_domain_8145/frontend/dist.backup_idle_rail_20260529_015339`; only `frontend/dist` was replaced, and `hackson-domain-8145.service` was not restarted. Local frontend build and targeted `git diff --check` passed. Public root served `/assets/index-B3JO7D8q.js` and `/assets/index-ZuQezEFt.css`; public browser smoke passed without starting a model turn and saved `scripts/artifacts/idle_right_rail_public.png`. Services `hackson-domain-8145.service` and `hackson-cloudflared.service` stayed active, and local plus public health checks returned `200`.
...
| GET | `/download/companion` | No | `backend/main.py` + `frontend/` | Public Desktop Companion Mac alpha download page. Local verification passed on 2026-05-28; public verification is recorded in the cleanup/deployment log after promotion. |
| GET | `/assets/{asset}` | No | `backend/main.py` | Built frontend assets |
| GET | `/assets/downloads/hackson-pet-mac-arm64.zip` | No | `backend/main.py` | Public Desktop Pet V0.8.4 Mac Apple Silicon alpha app zip. Local verification passed on 2026-05-28; public verification is recorded in the cleanup/deployment log after promotion. |
...
| POST | `/api/users/quick-try` | No | `backend/users/` | Create a real temporary user for Hackathon Quick Try and return the normal JWT auth shape. The public frontend stores this token in session storage so the trial is session-scoped. Verified on public `8145` on 2026-05-28. |
...
| POST | `/api/work/missions/{missionId}/instruction` | Bearer JWT | `backend/work_mode/` | Add a user instruction while a Mission is `running`; records `USER_INSTRUCTION_ADDED`, updates the Lead context for later turns, and does not create a new Run. Public route existence was verified on 2026-05-28 with the expected `409 mission_not_running` response for a draft Mission. |
